...
BEGIN
    SELECT s.id INTO SALE_STORAGE_ID FROM storage s JOIN orders o ON o.id = NEW.order_id WHERE o.accept_outlet_id = s.outlet_id;

    RAISE NOTICE 'Sale storage ID: %', SALE_STORAGE_ID;

    SELECT quantity INTO AVAILABLE_QUANTITY FROM storage_items si WHERE si.item_id = NEW.item_id AND si.storage_id = SALE_STORAGE_ID;
